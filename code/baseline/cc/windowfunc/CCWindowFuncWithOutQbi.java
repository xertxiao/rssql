...
public class CCWindowFuncWithOutQbi implements AllWindowFunction<ArcSlide, ArcSlide, TimeWindow> {
...
//	private static EDBQbi edbQbi = new EDBQbi();
    ...
    public void apply(TimeWindow timeWindow, Iterable<ArcSlide> arcs, Collector<ArcSlide> out) throws Exception {
						...
						for (ArcSlide a : arcs) {
							EDB.add(a);
							//idbQbi.insertAtom(a);
							minMap.put(a.getFromId(),
									new ArcSlide(a.getFromId(), a.getFromId(),
											a.getTimestamp(), a.getSlide(), a.getIteration()));
						}


						List<ArcSlide> ccList = new ArrayList<>(minMap.values());

						List<ArcSlide> IDB = loopJoinArcsCC(EDB, ccList);
						_ret.addAll(IDB);

						while (IDB.size() > 0) {
//							System.err.println("IDB Sie:" + IDB.size());
//							System.err.println("_ret Sie:" + _ret.size());
							IDB = loopJoinArcsCC(EDB, IDB);
							IDB.removeAll(_ret);
							_ret.addAll(IDB);
						}
