#include "experiments/cc_experiment.h"
...
namespace Experiment {
    CCExperiment::CCExperiment(unsigned int w, unsigned int s, bool q) {
        this->WINDOW_SLIDE = s;
        this->WINDOW_LEN = w;
        this->useQBI = q;
    ...
    int CCExperiment::registerSchema() {
...
//        for (auto p : ops) delete p;
    ...
    int CCExperiment::registerPhysical() {
        // Stream Source
...
//        rootOps->u.STREAM_SOURCE.strId = 1;
    ...
    int CCExperiment::runExperiment() {
        bool done = false;
